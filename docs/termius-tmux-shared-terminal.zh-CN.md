# 在手机上用 Termius + tmux 查看 Heart-Anchor 共享终端

这份文档是给不熟悉命令行的用户准备的。

目标只有一个：

- 让你在手机上随时查看 `Heart-Anchor` 当前绑定线程的运行情况
- 必要时在手机上接管同一条共享线程
- 尽量避免因为目录、runtime、tmux 用法不对而报错

这份文档默认你已经：

- 在一台长期在线的电脑或服务器上安装好了 `Heart-Anchor`
...
1. `npm run shared:start`
   这是共享桥主进程。它负责跑 Heart-Anchor 和微信桥。
...
很多报错都不是 Heart-Anchor 坏了，而是因为你不在项目目录里执行命令。
...
- 你以为自己在跑 `codex`
- 但环境变量里实际还是 `HEART_ANCHOR_RUNTIME=claudecode`
...
```bash
tmux new-session -d -s cb-open -c /Users/tingyiwen/Dev/cyberboss 'HEART_ANCHOR_RUNTIME=codex npm run shared:open; exec zsh'
```

方式 2：直接改 `.env`

```env
HEART_ANCHOR_RUNTIME=codex
```
...
如果你只是想在手机上“随时看一眼 Heart-Anchor 当前线程跑得怎么样”，最稳的思路不是反复新开窗口，而是：
