...
const { sendWeixinMediaFile, sendWeixinVoiceFile } = require("./media-send");
...
function createWeixinChannelAdapter(config) {
  ...
  return {
    ...
    async sendVoice({
      ...
      const resolvedToken = resolveContextToken(userId, contextToken);
      if (!resolvedToken) {
        throw new Error(`Missing context_token. Cannot send a voice message to user ${userId}.`);
      }
      return sendWeixinVoiceFile({
        filePath,
        to: userId,
        contextToken: resolvedToken,
        baseUrl: account.baseUrl,
