...
function mapClaudeCodeMessageToRuntimeEvent(message, raw) {
  ...
  switch (type) {
    ...
    case "process.error":
    case "process.close":
      if (type === "process.close" && Number(message?.code) === 0 && !normalizeString(message?.turnId)) {
        return null;
      }
      return {
        type: "runtime.turn.failed",
        payload: buildProcessFailurePayload(message, type),
      };
    case "session.id":
      return null;
    default:
...
function buildProcessFailurePayload(message, type) {
  const threadId = normalizeString(message?.sessionId);
  const turnId = normalizeString(message?.turnId);
  if (type === "process.error") {
    const detail = normalizeString(message?.error);
    return {
      threadId,
      turnId,
      failureKind: "process_error",
      text: detail
        ? `❌ Claude Code process error: ${detail}`
        : "❌ Claude Code process error",
    };
  }

  const exitCode = Number(message?.code);
  const hasExitCode = Number.isFinite(exitCode);
  const codeText = hasExitCode ? `code=${exitCode}` : "code=unknown";
  return {
    threadId,
    turnId,
    failureKind: "process_close",
    ...(hasExitCode ? { exitCode } : {}),
    text: `❌ Claude Code process exited unexpectedly (${codeText})`,
  };
}
