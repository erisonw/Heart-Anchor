...
class ClaudeCodeProcessClient {
  constructor({ command = "claude", cwd, env, model = "", permissionMode = "default", disableVerbose = false, extraArgs = [], mcpConfigPaths = [], ipcServer = null, workspaceRoot = "", assistantCompletionFallbackMs = 1500 }) {
    ...
    this.pendingToolUseInTurn = false;
    this.assistantCompletionFallbackMs = normalizeFallbackMs(assistantCompletionFallbackMs);
    this.assistantCompletionTimer = null;
  }
  ...
  async connect(resumeSessionId = "") {
    ...
    this.pendingAssistantText = "";
    this.pendingToolUseInTurn = false;
    this.clearAssistantCompletionTimer();
  ...
  handleAssistant(raw) {
    ...
    for (const item of content) {
      ...
      if (itemType === "text" && typeof item.text === "string" && item.text) {
        sawText = true;
        this.pendingAssistantText = item.text.trim();
        this.emit({
          type: "assistant.text",
          text: item.text.trim(),
          turnId: this.pendingTurnId,
        ...
        sawToolUse = true;
        this.pendingToolUseInTurn = true;
        this.clearAssistantCompletionTimer();
        this.emit({
          type: "tool.use",
          toolName,
    ...
    if (sawText && !sawToolUse) {
      this.pendingToolUseInTurn = false;
      this.scheduleAssistantCompletionFallback();
    }
  }
  ...
  handleResult(raw) {
    ...
    const fallbackText = resultText || this.pendingAssistantText;
    this.emit({
      type: "turn.completed",
      turnId: this.pendingTurnId,
      sessionId: this.activeThreadId || this.sessionId,
  ...
  scheduleAssistantCompletionFallback() {
    this.clearAssistantCompletionTimer();
    if (!this.assistantCompletionFallbackMs || !this.pendingTurnId || !this.pendingAssistantText) {
      return;
    }
    const turnId = this.pendingTurnId;
    const sessionId = this.activeThreadId || this.sessionId;
    this.assistantCompletionTimer = setTimeout(() => {
      this.assistantCompletionTimer = null;
      if (this.pendingTurnId !== turnId || this.pendingToolUseInTurn || !this.pendingAssistantText) {
        return;
      }
      this.emit({
        type: "turn.completed",
      ...
      this.pendingTurnId = "";
      this.activeThreadId = "";
    }, this.assistantCompletionFallbackMs);
    if (typeof this.assistantCompletionTimer.unref === "function") {
      this.assistantCompletionTimer.unref();
    }
  }

  clearAssistantCompletionTimer() {
    if (!this.assistantCompletionTimer) {
      return;
    }
    clearTimeout(this.assistantCompletionTimer);
    this.assistantCompletionTimer = null;
  }
...
function normalizeFallbackMs(value) {
