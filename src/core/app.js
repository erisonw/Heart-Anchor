...
const { AndroidWakeSuppressor, deriveAndroidTriggerKey } = require("./android-wake-suppressor");
...
const MAX_PROCESSED_INBOUND_MESSAGE_KEYS = 2_000;
...
class CyberbossApp {
  constructor(config) {
    ...
    this.androidWakeSuppressor = new AndroidWakeSuppressor();
  ...
  async start() {
    ...
    if (this.config.startWithAndroidWebhook) {
      await this.ensureAndroidWebhookStarted();
    }
    if (this.config.startWithWebConsole) {
      await this.ensureWebConsoleStarted();
    }
    ...
    const shutdown = createShutdownController(async () => {
      ...
      await this.closeAndroidWebhook();
      await this.closeWebConsole();
  ...
  async ensureAndroidWebhookStarted() {
    if (!this.projectServices?.androidIngest) {
      return null;
    }
    await this.projectServices.androidIngest.startServer({
  ...
  async closeAndroidWebhook() {
    if (!this.projectServices?.androidIngest) {
      return;
    }
    await this.projectServices.androidIngest.closeServer();
  }

  async ensureWebConsoleStarted() {
    if (this.webConsole) {
  ...
  async closeWebConsole() {
    if (!this.webConsole) {
      return;
    }
    const consoleServer = this.webConsole;
    this.webConsole = null;
    await consoleServer.close().catch(() => {});
  }

  async handleAndroidEventAccepted(result) {
    ...
    const target = this.resolvePreferredSystemTarget();
    if (!target) {
      return;
    }
    const triggerText = buildAndroidTriggerSystemText(result?.event);
    if (!triggerText) {
      return;
    }
    ...
    const eventId = normalizeCommandArgument(result?.event?.eventId) || normalizeCommandArgument(result?.event?.id);
    this.systemMessageQueue.enqueue({
      id: `android-trigger:${eventId || `${eventType}:${Date.now()}`}`,
      accountId: this.activeAccountId,
      senderId: target.senderId,
      workspaceRoot: target.workspaceRoot,
      text: triggerText,
      createdAt: normalizeIsoTime(result?.event?.occurredAt)
        || normalizeIsoTime(result?.event?.receivedAt)
  ...
  async writeAndroidEventToTimeline(event) {
    if (!this.projectServices?.timeline || !event || typeof event !== "object") {
      return;
    }
    const timelineWrite = buildAndroidTimelineWrite(event);
    if (!timelineWrite) {
      return;
    }
    try {
  ...
  async prepareIncomingMessageForRuntime(normalized, workspaceRoot) {
    ...
    const persisted = await persistIncomingAttachmentsForChannel({
      config: this.config,
      normalized,
      attachments,
    });
  ...
  resolveLongPollTimeoutMs() {
    if (normalizeCommandArgument(this.config.channel).toLowerCase() === "telegram"
      && Number.isFinite(Number(this.config.telegramPollTimeoutMs))) {
      return Math.max(0, Number(this.config.telegramPollTimeoutMs));
    }
  ...
  async dispatchChannelCommand(normalized, command) {
    switch (command.name) {
      ...
      case "chunk":
        ...
        return;
      case "remember":
        await this.handleRememberCommand(normalized, command);
        return;
      case "memory":
        await this.handleMemoryCommand(normalized, command);
        return;
      case "forget":
        await this.handleForgetCommand(normalized, command);
        return;
      case "yes":
      case "always":
      case "no":
  ...
  async handleRememberCommand(normalized, command) {
    const content = normalizeCommandArgument(command.args);
    if (!content) {
      await this.channelAdapter.sendText({
        userId: normalized.senderId,
    ...
    const result = this.projectServices.memory.remember({
      content,
      source: "command",
    });
    await this.channelAdapter.sendText({
  ...
  async handleMemoryCommand(normalized, command) {
    const args = splitCommandLine(command.args);
    const subcommand = normalizeCommandArgument(args[0]).toLowerCase();
    const id = normalizeCommandArgument(args[1]);
    if (subcommand === "approve") {
      if (!id) {
        await this.channelAdapter.sendText({
          userId: normalized.senderId,
          text: "💡 Usage: /memory approve <id>",
          contextToken: normalized.contextToken,
        });
        return;
      }
      const result = this.projectServices.memory.update({ id, status: "confirmed" });
      await this.channelAdapter.sendText({
        userId: normalized.senderId,
        text: `✅ Memory approved: ${result.id}`,
        contextToken: normalized.contextToken,
      });
      return;
    }
    if (subcommand === "reject") {
      if (!id) {
        await this.channelAdapter.sendText({
          userId: normalized.senderId,
          text: "💡 Usage: /memory reject <id>",
          contextToken: normalized.contextToken,
        });
        return;
      }
      const result = this.projectServices.memory.update({ id, status: "archived" });
      await this.channelAdapter.sendText({
        userId: normalized.senderId,
        text: `🗑️ Memory rejected: ${result.id}`,
        contextToken: normalized.contextToken,
      });
      return;
    }
    const status = subcommand && ["confirmed", "candidate", "archived"].includes(subcommand)
      ? subcommand
      : "candidate";
    const memories = this.projectServices.memory.list({ status, limit: 10 });
    await this.channelAdapter.sendText({
      userId: normalized.senderId,
      text: formatMemoryListText(memories, status),
      contextToken: normalized.contextToken,
      preserveBlock: true,
    });
  }

  async handleForgetCommand(normalized, command) {
    const id = normalizeCommandArgument(command.args);
    if (!id) {
      await this.channelAdapter.sendText({
        userId: normalized.senderId,
    ...
    const result = this.projectServices.memory.forget({ id });
    await this.channelAdapter.sendText({
      userId: normalized.senderId,
      text: `🗑️ Memory archived: ${result.id}`,
      contextToken: normalized.contextToken,
    });
  }
...
async function persistIncomingAttachmentsForChannel({ config, normalized, attachments }) {
  if (normalized?.provider === "telegram") {
    return persistIncomingTelegramAttachments({
      attachments,
      stateDir: config.stateDir,
      baseUrl: config.telegramApiBaseUrl,
      token: config.telegramBotToken,
      transport: config.telegramTransport,
      messageId: normalized.messageId,
      receivedAt: normalized.receivedAt,
    });
  }

  return persistIncomingWeixinAttachments({
    attachments,
    stateDir: config.stateDir,
...
function buildInboundMessageDedupeKey(normalized) {
  const provider = normalizeCommandArgument(normalized?.provider);
  const accountId = normalizeCommandArgument(normalized?.accountId);
  const senderId = normalizeCommandArgument(normalized?.senderId || normalized?.chatId);
  const messageId = normalizeCommandArgument(normalized?.messageId);
  if (!provider || !accountId || !senderId || !messageId) {
    return "";
  }
  return [provider, accountId, senderId, messageId].join(":");
}

function hasProcessedInboundMessageKey(map, key) {
  return Boolean(key && map instanceof Map && map.has(key));
}

function markProcessedInboundMessageKey(map, key) {
  if (!key || !(map instanceof Map)) {
    return;
  }
  map.set(key, Date.now());
  pruneProcessedInboundMessageKeys(map);
}

function pruneProcessedInboundMessageKeys(map) {
  if (!(map instanceof Map) || map.size <= MAX_PROCESSED_INBOUND_MESSAGE_KEYS) {
    return;
  }
  const overflow = map.size - MAX_PROCESSED_INBOUND_MESSAGE_KEYS;
  let removed = 0;
...
function formatMemoryListText(memories, status) {
  const items = Array.isArray(memories) ? memories : [];
  if (!items.length) {
    return `🧠 No ${status || "memory"} memories.`;
  }
  const lines = [`🧠 ${status || "memory"} memories:`];
  for (const memory of items.slice(0, 10)) {
    const tags = Array.isArray(memory.tags) && memory.tags.length
      ? ` [${memory.tags.join(", ")}]`
      : "";
    const content = String(memory.content || "").replace(/\s+/g, " ").trim();
    lines.push(`- ${memory.id}${tags}: ${content}`);
  }
  return lines.join("\n");
}
