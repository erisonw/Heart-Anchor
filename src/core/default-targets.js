...
function resolvePreferredSenderId({
  ...
  const configuredTelegramChats = normalizeText(config?.channel).toLowerCase() === "telegram"
    && Array.isArray(config?.telegramAllowedChatIds)
    ? config.telegramAllowedChatIds.map((value) => normalizeText(value)).filter(Boolean)
    : [];
  if (configuredTelegramChats.length) {
    return configuredTelegramChats[0];
  }
