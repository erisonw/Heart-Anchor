...
function sanitizeReplyText(plainReplyText) {
  ...
  const leak = detectReasoningLeak(trimmed);
  if (leak) {
...
const REASONING_LEAK_FIRST_LINE_PATTERNS = [
  /^th[A-Z][a-z]/,
  /^(?:Wait|Actually|Hmm|Hold on|Let me|Let's|I think|I'll|I'd|I'm|I should|I need to|I want to|I could|I can|I have to|Looking at|First[, ]|So |Okay,|OK,|This is (?:a|really|the))\b/i,
];
const REASONING_LEAK_THIRD_PERSON_PATTERNS = [
  /\b(?:he said|she said|the user|the user's|he asked|she asked|he wants|she wants|he mentioned|she mentioned|user mentioned)\b/i,
];
const ENGLISH_REASONING_MARKERS = /\b(?:I should|I need to|I want to|I'll|I will|I could|I can|I have to|I'm going to|I'd|I think|I praised|let me|let's|the user|he said|she said|he wants|she wants|he asked|she asked|he finished|he'd|she'd|this is (?:a|really|the)|closing the loop|relational moment|makes sense|note that|keep it|within chunk|backstop|first person|i receive)\b/i;

function stripEnglishReasoningParagraphs(text) {
  const normalized = trimOuterBlankLines(normalizeLineEndings(text));
  if (!normalized) {
    return "";
  }
  ...
  const kept = paragraphs.filter((paragraph) => !isEnglishReasoningParagraph(paragraph));
...
function isEnglishReasoningParagraph(paragraph) {
  const text = trimOuterBlankLines(paragraph);
  if (!text) {
    return false;
  }
  const cjkCount = (text.match(/[\u3400-\u9fff]/g) || []).length;
  const englishWordCount = (text.match(/[A-Za-z]{2,}/g) || []).length;
  if (englishWordCount < 8 || cjkCount > 6) {
    return false;
  }
  return ENGLISH_REASONING_MARKERS.test(text);
}
...
function detectReasoningLeak(text) {
  ...
  for (const pattern of REASONING_LEAK_FIRST_LINE_PATTERNS) {
    if (pattern.test(firstLine)) {
      return "first-line";
    }
  }
  for (const pattern of REASONING_LEAK_THIRD_PERSON_PATTERNS) {
...
function sanitizeReasoningLeakText(text) {
  const normalized = trimOuterBlankLines(normalizeLineEndings(text));
  if (!normalized) {
    return "";
  }
  if (!isStrongReasoningLeak(normalized)) {
    return normalized;
  }
  return extractLikelyFinalAnswer(normalized);
}

function isStrongReasoningLeak(text) {
  const firstLine = text.split(/\r?\n/, 1)[0].trim();
  if (/^th[A-Z][a-z]/.test(firstLine)) {
    return true;
  }
  const lower = text.toLowerCase();
  const mentionsUser = REASONING_LEAK_THIRD_PERSON_PATTERNS.some((pattern) => pattern.test(text));
  const hasReasoningPhrase = [
    "let me think",
    "let me check",
    "i should",
    "i need to",
    "the question is",
    "what tools/capability",
    "before promising",
    "answer honestly",
  ].some((phrase) => lower.includes(phrase));
  return mentionsUser && hasReasoningPhrase;
}
...
function extractLikelyFinalAnswer(text) {
  ...
  for (const paragraph of paragraphs) {
    if (!answerStarted) {
      const answerText = extractAnswerStartFromParagraph(paragraph);
      if (!answerText) {
        continue;
      }
      kept.push(answerText);
      answerStarted = true;
      continue;
    }
    if (isLikelyReasoningParagraph(paragraph)) {
      continue;
    }
    kept.push(paragraph);
  }
...
function splitParagraphs(text) {
  return trimOuterBlankLines(text)
    .split(/\n\s*\n/g)
    .map((paragraph) => trimOuterBlankLines(paragraph))
    .filter(Boolean);
}

function extractAnswerStartFromParagraph(paragraph) {
  ...
  for (const line of lines) {
    if (!collecting) {
      ...
      const candidate = line.slice(cjkIndex).trim();
      if (!candidate || isLikelyReasoningParagraph(candidate)) {
        continue;
      }
  ...
  return trimOuterBlankLines(answerLines.join("\n"));
}

function isLikelyReasoningParagraph(paragraph) {
  const normalized = trimOuterBlankLines(paragraph);
  if (!normalized) {
    return false;
  }
  const firstLine = normalized.split(/\r?\n/, 1)[0].trim();
  return REASONING_LEAK_FIRST_LINE_PATTERNS.some((pattern) => pattern.test(firstLine))
    || REASONING_LEAK_THIRD_PERSON_PATTERNS.some((pattern) => pattern.test(normalized))
    || /\b(?:The question is|before promising|what tools\/capability|answer honestly)\b/i.test(normalized);
}
