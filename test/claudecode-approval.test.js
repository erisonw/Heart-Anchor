...
test("claudecode adapter ignores provider-prefixed models from Claude project transcript", async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "cb-claude-provider-model-"));
  const stateDir = path.join(tempDir, "state");
  const claudeConfigDir = path.join(tempDir, "claude");
  const workspaceRoot = path.join(tempDir, "workspace root");
  const sessionId = "76767676-7676-4767-8767-767676767676";
  fs.mkdirSync(stateDir, { recursive: true });
  fs.mkdirSync(workspaceRoot, { recursive: true });
  const projectDir = path.join(claudeConfigDir, "projects", workspaceRoot.replace(/[\\/:\s]+/g, "-"));
  fs.mkdirSync(projectDir, { recursive: true });
  fs.writeFileSync(path.join(projectDir, `${sessionId}.jsonl`), [
    JSON.stringify({ type: "assistant", message: { model: "anthropic/claude-4.6-sonnet-20260217" } }),
  ].join("\n"));
  const sessionsFile = path.join(tempDir, "sessions.json");
  new SessionStore({ filePath: sessionsFile, runtimeId: "claudecode" })
    .setThreadIdForWorkspace("binding-1", workspaceRoot, sessionId);
...
test("claudecode assistant events map usage into context snapshots", () => {
  ...
  assert.equal(event.payload.inputTokens, 7);
  assert.equal(event.payload.cacheCreationInputTokens, 12150);
  assert.equal(event.payload.cacheReadInputTokens, 13535);
...
test("claudecode process client treats assistant text as non-deliverable until the result event", () => {
  const client = new ClaudeCodeProcessClient({
    ...
    cwd: "/workspace",
    env: {},
    assistantCompletionFallbackMs: 0,
  });
...
test("claudecode process client completes plain assistant text after quiet fallback", async () => {
  const client = new ClaudeCodeProcessClient({
    command: "claude",
    cwd: "/workspace",
    env: {},
    assistantCompletionFallbackMs: 10,
  });
  client.pendingTurnId = "turn-plain";
  client.sessionId = "thread-plain";
  client.activeThreadId = "thread-plain";
  const messages = [];
  client.onMessage((event, raw) => {
    messages.push({ event, raw });
  });
  ...
  const completed = mapClaudeCodeMessageToRuntimeEvent(messages[1].event, messages[1].raw);
  assert.equal(completed.type, "runtime.turn.completed");
  assert.equal(completed.payload.threadId, "thread-plain");
  assert.equal(completed.payload.turnId, "turn-plain");
  assert.equal(completed.payload.text, "收到～");
});

test("claudecode process client does not fallback complete assistant text before a tool result", async () => {
  const client = new ClaudeCodeProcessClient({
    command: "claude",
    cwd: "/workspace",
    env: {},
  ...
  client.pendingTurnId = "turn-tool-wait";
  client.sessionId = "thread-tool-wait";
  client.activeThreadId = "thread-tool-wait";
  const messages = [];
  client.onMessage((event, raw) => {
    messages.push({ event, raw });
  });
...
test("claudecode process client completes final assistant text after a stuck tool-use state", async () => {
  const client = new ClaudeCodeProcessClient({
    command: "claude",
    cwd: "/workspace",
    env: {},
  ...
  client.pendingTurnId = "turn-stuck-tool";
  client.sessionId = "thread-stuck-tool";
  client.activeThreadId = "thread-stuck-tool";
  client.pendingToolUseInTurn = true;
  const messages = [];
  client.onMessage((event, raw) => {
    messages.push({ event, raw });
  });
  ...
  const completed = mapClaudeCodeMessageToRuntimeEvent(messages[1].event, messages[1].raw);
  assert.equal(completed.type, "runtime.turn.completed");
  assert.equal(completed.payload.threadId, "thread-stuck-tool");
  assert.equal(completed.payload.turnId, "turn-stuck-tool");
  assert.equal(completed.payload.text, "下单好了，去付款就行。");
});

test("claudecode process client falls back to assistant text when result text is empty", () => {
  const client = new ClaudeCodeProcessClient({
    command: "claude",
    cwd: "/workspace",
    env: {},
  });
  client.pendingTurnId = "turn-system";
  client.sessionId = "thread-system";
  client.activeThreadId = "thread-system";
  const messages = [];
  client.onMessage((event, raw) => {
    messages.push({ event, raw });
  });
  ...
  client.handleAssistant({
    ...
    message: {
      content: [
        { type: "thinking", thinking: "I should wake him up." },
        { type: "text", text: "{\"action\":\"send_message\",\"message\":\"起床了\"}" },
      ],
    },
  });
  client.handleResult({
    type: "result",
    session_id: "thread-system",
    result: "",
  });

  const completed = messages.find((entry) => entry.event.type === "turn.completed");
  assert.ok(completed);
  assert.equal(completed.event.text, "{\"action\":\"send_message\",\"message\":\"起床了\"}");
  ...
  assert.equal(mapped.payload.text, "{\"action\":\"send_message\",\"message\":\"起床了\"}");
});

test("claudecode process client ignores duplicate result events after a turn is completed", () => {
  const client = new ClaudeCodeProcessClient({
    command: "claude",
    cwd: "/workspace",
    env: {},
  });
  client.pendingTurnId = "turn-dup";
  client.sessionId = "thread-dup";
  client.activeThreadId = "thread-dup";
  const messages = [];
  client.onMessage((event, raw) => {
    messages.push({ event, raw });
  });
  ...
  const completed = messages.filter((entry) => entry.event.type === "turn.completed");
  assert.equal(completed.length, 1);
  assert.equal(completed[0].event.turnId, "turn-dup");
  assert.equal(completed[0].event.text, "第一次结果");
});

test("claudecode process close code 0 without a pending turn is ignored", () => {
  ...
  assert.equal(activeClose.payload.threadId, "thread-active");
  assert.equal(activeClose.payload.turnId, "turn-active");
});

test("claudecode process errors carry diagnostic failure metadata", () => {
  const event = mapClaudeCodeMessageToRuntimeEvent({
    type: "process.error",
  ...
  assert.equal(event.payload.threadId, "thread-error");
  assert.equal(event.payload.turnId, "turn-error");
  assert.equal(event.payload.failureKind, "process_error");
  assert.match(event.payload.text, /spawn claude ENOENT/);
});

test("claudecode process close events carry exit code diagnostics", () => {
  const event = mapClaudeCodeMessageToRuntimeEvent({
    type: "process.close",
    code: 1,
  ...
  assert.equal(event.type, "runtime.turn.failed");
  assert.equal(event.payload.threadId, "thread-close");
  assert.equal(event.payload.turnId, "turn-close");
  assert.equal(event.payload.failureKind, "process_close");
  assert.equal(event.payload.exitCode, 1);
  assert.match(event.payload.text, /code=1/);
});
...
test("handleStatusCommand shows approximate context details for claudecode when configured", async () => {
  ...
  assert.match(sent[0], /cache 72% hit \(13k read\/3k write\)/);
});
