...
const skipWithoutSips = process.platform === "darwin"
  ? false
  : "sticker GIF normalization for non-GIF inputs requires macOS sips";

test("sticker service saves inbox images as GIF stickers, grows tags, dedupes, and notifies once", { skip: skipWithoutSips }, async () => {
  const config = createConfig();
...
test("sticker service saves inbox images from an items array and keeps the tag catalog deduped", { skip: skipWithoutSips }, async () => {
  const config = createConfig();
  const { service, sentTexts } = createService(config);
...
test("sticker service updates, picks, sends, and deletes saved stickers", { skip: skipWithoutSips }, async () => {
  const config = createConfig();
