...
test("plain telegram reply strips timestamped user context leak", async () => {
  const { sent, streamDelivery } = createHarness();
  streamDelivery.queueReplyTargetForThread("thread-4tg", {
    userId: "5001000001",
    contextToken: "tg:5001000001",
    provider: "telegram",
  });
  ...
  assert.deepEqual(sent[0], {
    userId: "5001000001",
    text: "哪里难受，鼻子堵得睡不着还是头疼？",
    contextToken: "tg:5001000001",
  });
});

test("plain telegram reply strips claudecode reasoning leak and keeps final answer", async () => {
  const { sent, streamDelivery } = createHarness();
  streamDelivery.queueReplyTargetForThread("thread-reasoning-tail", {
    userId: "5001000001",
    contextToken: "tg:5001000001",
    provider: "telegram",
  });
  ...
  await runCompletedTurn(streamDelivery, {
    threadId: "thread-reasoning-tail",
    turnId: "turn-reasoning-tail",
    itemId: "item-reasoning-tail",
    text: [
      "thHe's asking a technical question: can the current agent editing of doc documents also paste in images he provides.",
      "",
      "The question is whether I can produce a .doc/.docx with his images embedded.",
      "",
      "Let me give a practical answer: yes, it's doable.",
      "",
      "Actually the cleanest path is to check pandoc / python-docx before promising.",
      "能，技术上做得到——我可以把你的文字和图片一起生成一个 .docx，图片嵌进正文对应位置。",
      "",
      "我先看看这边环境支持哪条路。",
    ].join("\n"),
  });
  ...
  assert.deepEqual(sent[0], {
    userId: "5001000001",
    text: "能，技术上做得到——我可以把你的文字和图片一起生成一个 .docx，图片嵌进正文对应位置。\n\n我先看看这边环境支持哪条路。",
    contextToken: "tg:5001000001",
  });
});

test("plain telegram reply suppresses pure claudecode reasoning leak", async () => {
  const { sent, streamDelivery } = createHarness();
  streamDelivery.queueReplyTargetForThread("thread-reasoning-only", {
    userId: "5001000001",
    contextToken: "tg:5001000001",
    provider: "telegram",
  });
  ...
  await runCompletedTurn(streamDelivery, {
    ...
    itemId: "item-reasoning-only",
    text: [
      "thHe's asking a technical question about document generation.",
      "",
      "The question is whether I can produce a .doc/.docx with embedded images.",
      "",
      "Let me think about what tools/capability exist here.",
    ].join("\n"),
  });
...
test("plain telegram reply suppresses CJK-led first-person english reasoning leak", async () => {
  const { sent, streamDelivery } = createHarness();
  streamDelivery.queueReplyTargetForThread("thread-cjk-reasoning", {
    userId: "5001000001",
    contextToken: "tg:5001000001",
    provider: "telegram",
  });
  ...
  await runCompletedTurn(streamDelivery, {
    threadId: "thread-cjk-reasoning",
    turnId: "turn-cjk-reasoning",
    itemId: "item-cjk-reasoning",
    text: [
      "心动 moment was when I praised him after he finished the memory system. This is a real, warm relational moment. I should receive it warmly, with my own voice. I could propose or commit it to long-term memory. He'd love that, closing the loop.",
    ].join("\n"),
  });
...
test("plain telegram reply keeps chinese answer and drops trailing english reasoning paragraph", async () => {
  const { sent, streamDelivery } = createHarness();
  streamDelivery.queueReplyTargetForThread("thread-cjk-keep", {
    userId: "5001000001",
    contextToken: "tg:5001000001",
    provider: "telegram",
  });
  ...
  await runCompletedTurn(streamDelivery, {
    threadId: "thread-cjk-keep",
    turnId: "turn-cjk-keep",
    itemId: "item-cjk-keep",
    text: [
      "我在呢，别想太多，先睡。",
      "",
      "I should keep this short and warm. Let me make sure I don't over-explain or push him. This is a settling-down moment for him.",
    ].join("\n"),
  });
...
test("plain telegram reply does not strip a short english song title in a chinese reply", async () => {
  const { sent, streamDelivery } = createHarness();
  streamDelivery.queueReplyTargetForThread("thread-song-title", {
    userId: "5001000001",
    contextToken: "tg:5001000001",
    provider: "telegram",
  });
  ...
  await runCompletedTurn(streamDelivery, {
    threadId: "thread-song-title",
    turnId: "turn-song-title",
    itemId: "item-song-title",
    text: "放给你了，Listening for the Weather，Bic Runga 的原版，整首。戴耳机听更好。",
  });
