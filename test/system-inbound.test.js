...
test("handleIncomingMessage ignores duplicate inbound message ids", async () => {
  const handled = [];
  const appLike = {
    processedInboundMessageKeys: new Map(),
    channelAdapter: {
  ...
  await CyberbossApp.prototype.handleIncomingMessage.call(appLike, message);
  await CyberbossApp.prototype.handleIncomingMessage.call(appLike, { ...message });
...
test("runtime turn includes bounded confirmed long-term memory context", async () => {
  const runtimeTurn = await CyberbossApp.prototype.buildRuntimeTurn.call({
    config: {
      userName: "User",
    },
    ...
    prepared: {
      provider: "telegram",
      originalText: "空调开太低我睡不着",
      text: "空调开太低我睡不着",
      attachments: [],
      attachmentFailures: [],
      receivedAt: "2026-06-28T06:00:00.000Z",
    },
    model: "claude-sonnet",
  });

  assert.match(runtimeTurn.text, /Relevant long-term memory:/);
  assert.match(runtimeTurn.text, /浩浩怕冷/);
  assert.match(runtimeTurn.text, /空调开太低我睡不着/);
  assert.doesNotMatch(runtimeTurn.text, /memory\.sqlite/);
  assert.doesNotMatch(runtimeTurn.text, /score/i);
});

test("runtime turn omits memory context when no confirmed memory matches", async () => {
  const runtimeTurn = await CyberbossApp.prototype.buildRuntimeTurn.call({
    config: {
      userName: "User",
    },
  ...
  assert.doesNotMatch(runtimeTurn.text, /Relevant long-term memory:/);
  assert.match(runtimeTurn.text, /今天吃什么/);
});
...
test("watch heart rate alert writes timeline and enqueues a system action message", async () => {
  const queued = [];
  const timelineWrites = [];
  await CyberbossApp.prototype.handleAndroidEventAccepted.call({
    activeAccountId: "wx-account",
    resolvePreferredSystemTarget() {
      return {
        senderId: "user-1",
    ...
    writeAndroidEventToTimeline: CyberbossApp.prototype.writeAndroidEventToTimeline,
  }, {
    event: {
      eventId: "hr-1",
      deviceId: "watch-main",
      ...
      receivedAt: "2026-07-01T08:00:01+08:00",
      payload: {
        alertKind: "high",
        bpm: 128,
      },
      summary: "watch heart rate high 128 bpm",
    },
  });

  assert.equal(queued.length, 1);
  assert.equal(queued[0].id, "android-trigger:hr-1");
  assert.match(queued[0].text, /heart rate/i);
  assert.match(queued[0].text, /128/);
  assert.equal(timelineWrites.length, 1);
  assert.equal(timelineWrites[0].date, "2026-07-01");
  assert.equal(timelineWrites[0].events[0].categoryId, "health");
  assert.equal(timelineWrites[0].events[0].subcategoryId, "health.other");
  assert.equal(timelineWrites[0].events[0].title, "Watch heart rate alert: 128 bpm");
});

test("healthy watch sleep summary writes timeline but does not enqueue a system action", async () => {
  const queued = [];
  const timelineWrites = [];
  await CyberbossApp.prototype.handleAndroidEventAccepted.call({
    activeAccountId: "wx-account",
    ...
    event: {
      ...
      receivedAt: "2026-07-01T07:30:01+08:00",
      payload: {
        totalSleepMinutes: 460,
        sleepScore: 82,
      },
      summary: "watch sleep 7h 40m score 82",
    },
  });
  ...
  assert.equal(timelineWrites.length, 1);
  assert.equal(timelineWrites[0].date, "2026-07-01");
  assert.equal(timelineWrites[0].events[0].categoryId, "rest");
  assert.equal(timelineWrites[0].events[0].subcategoryId, "rest.sleep");
  assert.equal(timelineWrites[0].events[0].eventNodeId, "evt.sleep");
  assert.equal(timelineWrites[0].events[0].title, "Watch sleep summary: 7h 40m");
});
